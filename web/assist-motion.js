import {normalizeRegion} from './motion-region.js';
...
export const motionFields={tailSwing:range(0,30),tailCycles:{type:"integer",min:1,max:4},irisGaze:{type:'enum',options:['natural','sweep']},earPattern:{type:'enum',options:['twitch','double','alternate','droop','up','natural']},earCycles:{type:'integer',min:1,max:6},independentHair:{type:'boolean'},frontHairMethod:{type:'enum',options:['wave','spring','off']},backHairMethod:{type:'enum',options:['wave','spring','off']},frontHairCycles:{type:'integer',min:1,max:4},backHairCycles:{type:'integer',min:1,max:4},irisX:range(0,20),irisScale:range(0,10),irisCycles:{type:"integer",min:1,max:4},duration:range(1,30),sway:range(0,12),breathe:range(0,40),
 ...
 headIdle:{type:'boolean'},headYawOffset:range(-1,1),headRollOffset:range(-8,8),headPitch:range(-1,1),pitchSway:range(0,1),headYaw:range(0,1),headTilt:range(0,8),headNod:range(0,6),bodyFollow:range(0,1),
 ...
 faceCoordination:{type:'boolean'},faceNeckBlend:range(.15,.6),chestMotionRegion:{type:'region'},chestRegionManual:{type:'boolean'},chestCenterX:range(0,1),chestCenterY:range(0,1),chestRadiusX:range(.01,.5),chestRadiusY:range(.01,.5),
...
export function validateMotion(values){
 ...
 for(const [key,v]of Object.entries(values)){
  ...
  const valid=f.type==='region'?(v===null||!!normalizeRegion(v)):f.type==='boolean'?typeof v==='boolean':f.type==='enum'?f.options.includes(v):typeof v==='number'&&Number.isFinite(v)&&v>=f.min&&v<=f.max&&(f.type!=='integer'||Number.isInteger(v));
